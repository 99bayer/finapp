...
import os, json, hmac, hashlib, smtplib
...
class MesFinanceiro(db.Model):
    ...
    def to_dict(self):
        return {
            "id": self.id,
            "mes": self.mes,
            "meta_nec": self.meta_nec,
            "meta_laz": self.meta_laz,
            "meta_inv": self.meta_inv,
...
def email_autorizado(email):
    reg = EmailAutorizado.query.filter_by(
...
def validar_token_reset(token, max_age=3600):
    s = get_serializer()
...
def enviar_email_reset(destinatario, nome, link):
    if not SMTP_HOST or not SMTP_USER or not SMTP_PASS:
        print("SMTP não configurado. Link de reset:", link)
        return

    assunto = "Redefinição de senha — Financeiro Pessoal"
    corpo = f"""Olá, {nome or 'usuário'}!
...
def calcular_dashboard(data):
    ...
    total_rend = sum(
        float(i.get("atual", 0)) - float(i.get("aplicado", 0))
        for i in invs if i.get("atual") and i.get("aplicado")
    )
    total_saldo_dev = sum(
        float(p.get("faltam", 0)) * float(p.get("valor_parcela", 0))
        for p in parcelas
    )
    ...
    alertas = [
        {
            ...
            "sub": "Fixas+Parcelas/Receita",
            "atual": pct(total_fix + total_parc, total_rec),
            "limite": meta_nec * 100,
            "ok": (total_fix + total_parc) <= total_rec * meta_nec,
            "unidade": "%",
            "diag_ok": "Dentro do limite",
            "diag_warn": "Acima do limite — revisar fixas"
        },
        {
            "icon": "🛒",
            "label": "Lazer e variáveis",
            "sub": "Variáveis/Receita",
            "atual": pct(total_var, total_rec),
            "limite": meta_laz * 100,
            "ok": total_var <= total_rec * meta_laz,
            "unidade": "%",
            "diag_ok": "Lazer equilibrado",
            "diag_warn": "Variáveis altos — cortar gastos"
        },
        {
            "icon": "📈",
            ...
            "limite": meta_inv * 100,
            "ok": total_apo >= total_rec * meta_inv,
            "unidade": "%",
            "diag_ok": "Investindo o recomendado",
            "diag_warn": "Aporte abaixo do ideal"
        },
        {
            "icon": "💳",
            "label": "Parcelas vs renda",
            "sub": "Parcelas/Receita",
            "atual": pct(total_parc, total_rec),
            "limite": 30,
            "ok": total_parc <= total_rec * 0.3,
            "unidade": "%",
            "diag_ok": "Parcelas controladas",
            "diag_warn": "Parcelas comprometem a renda"
        },
        {
            "icon": "💰",
            "label": "Sobra líquida",
            ...
            "limite": 10,
            "ok": sobra >= total_rec * 0.1,
            "unidade": "%",
            "diag_ok": "Margem saudável",
            "diag_warn": "Sobra muito baixa"
        },
    ]
    ...
    return {
        ...
        "sobra": sobra,
        "total_pat": total_pat,
        "total_rend": round(total_rend, 2),
        "total_apo": total_apo,
        "total_saldo_dev": total_saldo_dev,
        "alertas": alertas,
        "alertas_warn": sum(1 for a in alertas if not a["ok"]),
        "sobra_pos": sobra_pos,
...
def webhook_kiwify():
    ...
    try:
        ...
        status_validos = {
            "approved", "paid", "order_approved", "order_paid",
        ...
        plano = (
            payload.get("product", {}).get("name") or
            payload.get("plan_name") or
            payload.get("offer_name", "Plano Padrão")
        )
...
# ── ADMIN: liberar e-mail manualmente ────────────────────────────────────────
    ...
    if request.method == "POST":
        ...
        if not email_autorizado(email):
            return render_template(
                "cadastro.html",
                erro="Este e-mail não possui acesso autorizado. Use o mesmo e-mail da compra ou entre em contato pelo Instagram."
            )
...
def esqueci_senha():
    if request.method == "POST":
        ...
        if email:
            u = Usuario.query.filter_by(email=email).first()
            if u:
                token = gerar_token_reset(u.email)
                link = f"{APP_URL}/resetar-senha/{token}"
                enviar_email_reset(u.email, u.nome, link)

        return render_template("esqueci_senha.html", sucesso=mensagem, erro=None)

    return render_template("esqueci_senha.html", sucesso=None, erro=None)


@app.route("/resetar-senha/<token>", methods=["GET", "POST"])
def resetar_senha(token):
    try:
        email = validar_token_reset(token, max_age=3600)
    except SignatureExpired:
        return render_template(
            "resetar_senha.html",
            erro="Este link expirou. Solicite uma nova redefinição.",
            sucesso=None,
            token=None
        )
    except BadSignature:
    ...
    if request.method == "POST":
        ...
        return render_template(
            "resetar_senha.html",
            erro=None,
            sucesso="Senha redefinida com sucesso. Agora você já pode entrar.",
            token=None
        )

    return render_template("resetar_senha.html", erro=None, sucesso=None, token=token)
